classdef RealTimeAveragePlot < TMSi.HiddenHandle
    %REALTIMEPLOT Provides a very basic visualization of the event related averaged data that can be sampled from a device.
    %Altered from RealTimePlot provided by TMSi
    ...
    %
    %RealTimeAveragePlot Properties:
    ...
    %       RealTimeAveragePlot.append(samples);
    %   end
    %   sampler.stop();
    %   sampler.disconnect();
    ...
    properties (SetAccess = private)
        ...
        % The event type; either LEP or CHEPs
        event
    ...
    methods
        function obj = RealTimeAveragePlot(name, sample_rate, channels)
            %REALTIMEPLOT - Constructor for the RealTimePlot class.
            %
        ...
        function append(obj, samples)
            ...
            if size(samples, 2) > 0
                if size(samples, 1) > numel(obj.channels)
                    throw(MException('RealTimeAveragePlot:append', 'Too many channels.'));
                end
